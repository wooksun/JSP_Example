	...
	<c:if test="${list.size() != 0}">
		...
		<tr>
			<td colspan="5" align="center">
				...
				<c:if test="${boardList.currentPage <= 1}"> <!-- else와 같은 의미 -->
					<button 
						type="button" 
						disabled="disabled" 
						title="이미 첫 페이지 입니다."
				...
				<c:if test="${boardList.startPage <= 1}"> <!-- else와 같은 의미 -->
					<button 
						type="button" 
						disabled="disabled" 
				...
				<c:forEach var="i" begin="${boardList.startPage}" end="${boardList.endPage}" step="1">
					...
					<c:if test="${boardList.currentPage != i}">
						<button type='button' title="${i}페이지로 이동합니다." onclick="location.href='?currentPage=${i}'">${i}</button>
					</c:if>
				</c:forEach>

...
public class CategoryDAO {
	...
	//	CategoryService 클래스에서 호출되는 mapper와 삭제할 카테고리 정보가 저장된 객체를 넘겨받고, 카테고리 목록을 얻어오는 
	//	category.xml 파일의 select sql 명령을 실행하는 메소드
	...
	//	CategoryService 클래스에서 호출되는 mapper와 seq를 다시 부여할 카테고리 그룹을 넘겨받고, 삭제한 카테고리 그룹에 해당되는
	//	카테고리 목록을 얻어오는 category.xml 파일의 select sql 명령을 실행하는 메소드
	...
	//	CategoryService 클래스에서 호출되는 mapper와 seq를 다시 부여할 카테고리 일련번호와 수정할 데이터가 저장된  

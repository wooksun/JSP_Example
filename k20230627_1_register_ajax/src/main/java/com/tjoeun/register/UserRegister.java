...
public class UserRegister extends HttpServlet {
	...
	protected void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (userID == null || userID.equals("") || 
			...
			//response.sendRedirect("index.jsp"); // 1번 작업
			
			//	입력 체크 성공 여부에 따른 메시지를 ajax로 넘겨준다.
			response.getWriter().write("1"); // 2번작업
		...
		if (!userPassword1.equals(userPassword2)) {
			...
			//response.sendRedirect("index.jsp"); // 1번작업
			
			//	비밀번호 체크 성공 여부에 따른 메시지를 ajax로 넘겨준다.
			response.getWriter().write("2"); // 2번작업
			return;
		}
		...
		*/
		...
		if (result == 1) {
			...
			response.getWriter().write("3"); // 2번작업
		} else {
			...
			response.getWriter().write("4"); // 2번작업
		}
		//response.sendRedirect("index.jsp"); // 1번작업
	}

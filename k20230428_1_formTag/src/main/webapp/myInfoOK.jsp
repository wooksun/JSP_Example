...
// form에 입력된 한글 데이터가 post방식으로 전송될 때 깨지는 현상을 방지한다.
// 한글 깨짐을 방지하려면 최초의 request.getParameter() 메소드가 실행되기 전(맨처음)에 아래의 내용을 코딩한다.
	request.setCharacterEncoding("UTF-8");

// get방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설장해야 한다.
// Servers 폴더의 server.xml 파일을 열고 Connector에 URLEncoding="UTF-8"을 입력한다.
...
// 이전페이지에서 넘어오는 데이터는 무조건 문자열 형태로 넘어온다.
	...
	try {
		...
		out.println(name + "님의 내년 나이는" + (age + 1) + "살 입니다.<br>");
...
//	checkbox에서 넘어오는 데이터를 받을 때 넘어오는 항목이 여러개일 수 있기 때문에 getParameterValues() 메소드로 받아서 배열에 저장해야 한다.
	String[] hobbies = request.getParameterValues("hobbies"); //getParameterValues 받아 배열로 저장
	out.println(name + "님의 취미는 " + hobbies + "입니다.<br>"); 
	out.println(name + "님의 취미는 " + Arrays.toString(hobbies) + "입니다.<br>"); 
	out.println(name + "님의 취미는 "); 
	try{
		for(int i=0; i<hobbies.length; i++){ //반복으로 처리
	...
	String[] travel = request.getParameterValues("travel"); //getParameterValues 받아 배열로 저장
	out.println(name + "님은  " + travel + "에 가고싶어 합니다.<br>"); 
	out.println(name + "님은 " + Arrays.toString(travel) + "에 가고싶어합니다.<br>"); 
	out.println(name + "님의 취미는 "); 
	try{
		for(int i=0; i<travel.length; i++){ //반복으로 처리
			out.println(travel[i] + " ");
	}
		out.println("에 가고싶어합니다.<br>");
	} catch(NullPointerException e) {
		out.println("가고싶어하는 곳이 없습니다.<br>");
	}
